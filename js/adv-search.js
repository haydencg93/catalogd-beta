...
import {
    ...
    checkProviders,
    buildDiscoverUrls
} from './logic/adv-search-logic.js';
...
async function executeSearch(isLoadMore = false) {
    ...
    try {
        const requests = buildDiscoverUrls(activeTypes, textQuery, filters, currentPage, PROXY_URL);
...
function evaluateItemDetail(item, detailData, filters) {
    ...
    return checkDuration(item, detailData) && 
           ...
           checkProviders(detailData, filters.includeFree, Array.from(activeProviders)) &&
           checkYearLocally(item, filters); 
}
